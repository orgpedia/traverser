...
def start(url, log_file):
    return Traverser(url, log_file)
...
class Traverser:
    ...
    def _build_table(self, table):
        ...
        for idx, row in enumerate(table.query_selector_all("tr")):
    ...
    def save_screenshot(self, file_path, full_page=True, zoom_percent="100%"):
        zoom_percent = str(zoom_percent)
        zoom_percent = zoom_percent if zoom_percent[-1] == '%' else f'{zoom_percent}%'
        if zoom_percent != "100%":
            self.page.evaluate("document.body.style.zoom = '" + zoom_percent + "'")
